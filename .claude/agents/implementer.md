...
- **This is a bit-exact porting repo.** Read docs/PORTING_PLAN.md and CLAUDE.md before any step; when porting a Rust module, read the Rust source in full first, port its header and invariant comments verbatim, and build a scratchpad differential harness against the compiled manifold-rust whenever the step's tests alone cannot prove bit-exactness.
