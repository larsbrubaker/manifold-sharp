...
```bash
# Run all tests (from the repo root; --project is load-bearing)
dotnet test --project ManifoldSharp.Tests/ManifoldSharp.Tests.csproj

# Run a specific test class
dotnet test --project ManifoldSharp.Tests/ManifoldSharp.Tests.csproj -- --treenode-filter "/*/*/ClassName/*"

# Run a specific test method
dotnet test --project ManifoldSharp.Tests/ManifoldSharp.Tests.csproj -- --treenode-filter "/*/*/ClassName/MethodName"
```

> **Important:** Avoid `dotnet test --filter` with TUnit; use `--treenode-filter` after `--`. A positional project path (without `--project`) silently falls back to the cwd solution.
...
```bash
dotnet test --project ManifoldSharp.Tests/ManifoldSharp.Tests.csproj -- --treenode-filter "/*/*/ClassName/TestMethod"
```
...
2. Run the full test suite: `dotnet test --project ManifoldSharp.Tests/ManifoldSharp.Tests.csproj` (Debug and Release)
...
```
ManifoldSharp.Tests/
  ManifoldSharp.Tests.csproj     # TUnit test project (net10.0)
  <Module>Tests.cs               # one file per ported Rust test module
  TestData/thingi10k/            # STL fixtures (Phase 10 robust suite)
ManifoldSharp.OracleTests/       # bit-for-bit comparison vs the native ManifoldRust binding
```

**Porting-specific rule:** a failing ported test is NEVER fixed by changing its expected
value to what the C# produces — the Rust's value is the specification. Diff intermediates
against the compiled manifold-rust (the differential-harness pattern in the scratchpad)
until the diverging operation is found.

**Key patterns:**
- Standard tests: `[Test]` attribute, async Task methods
- Automation tests: Inherit `AutomationTestBase`, use `[NotInParallel]`
